...
local scales = {
  ...
  { name = "Major ♭6",              notes = {0, 2, 4, 5, 7, 8, 11} },
  { name = "Minor ♭6",              notes = {0, 2, 3, 5, 7, 8, 10} },
  { name = "Lydian ♯4",             notes = {0, 2, 4, 6, 7, 9, 10} },
...
local probs = {}  -- Probability weights for scale notes
...
local function compute_probabilities(scale)
  ...
  for i, n in ipairs(scale) do
    ...
    for _, m in ipairs(major) do
      if m == n then is_char = false break end
    end
...
local function weighted_pick(scale, probs)
  local r = math.random()
  local accum = 0
  for i, p in ipairs(probs) do
    accum = accum + p
    if r <= accum then return scale[i] end
  end
  return scale[#scale]
end

local function initialize_patterns(scale, probs, length)
  -- Initialize note pattern with random notes from scale
  note_pattern = {}
  for i = 1, length do
    note_pattern[i] = weighted_pick(scale, probs)
  end
  
  -- Initialize gate pattern with random gate states (50% chance)
  gate_pattern = {}
  for i = 1, length do
...
return {
  ...
  gate = function(self, input, rising)
    if input == 1 and rising then
      ...
      if #note_pattern ~= length then
        probs = compute_probabilities(scale)
        initialize_patterns(scale, probs, length)
      end
      ...
      if math.random() < note_prob then
        note_pattern[current_step] = weighted_pick(scale, probs)
      end
  ...
  trigger = function(self, input)
    if input == 2 then current_step = 0 end
  end,
  ...
  encoder1Turn = function(self, d)
    ...
    if scale_index < 1 then scale_index = #scales
    elseif scale_index > #scales then scale_index = 1 end
    setParameter(alg, self.parameterOffset + SCALE_PARAM, scale_index)
    probs = compute_probabilities(scales[scale_index].notes)
  end,
  ...
  encoder2Turn = function(self, d)
    ...
    if r < 0 then r = 11 
    elseif r > 11 then r = 0 end
    setParameter(alg, self.parameterOffset + ROOT_PARAM, r)
  end,

  button2Push = function() exit() end,
